...
`BANKSYS`), `LOAN`, `GBOND`, `RES` (liability of `CB`), `EQ:<issuer>`. Real assets (`CAPITAL`, `INVENTORY`,
...
split `RES` = 8 % of deposits, rest `GBOND`; `CB` holds `GBOND` = `RES`; households hold all deposits, the remaining
bonds and all equity (at book value in Phase 2). Consumption-relevant wealth `W = DEP_hh + GBOND_hh`.
...
debt_j    = nd_ebitda_j · 12 · EBITDA0_j ;  spread_j = base_spread · max(nd_j, 0.2) / 2.5
...
**Central bank.** Quarterly: `target = r_n + π* + φ_π(π_pol − π*) + φ_y·gap`, `π_pol = ½π3 + ½π12` (optionally a
...
cc_gap_j = (r − π_e − r_n)·100 + (nd_j/2.5)·Δspread_j·100 + Δerp·100         # in 100bp
...
lowers the annual start rate by 0.45 pp (from δ = 6 pp). The prototype scaled the *whole* `cc_gap` by
`max(nd,0.5)/2.5`; the form above scales only the spread term (faithful to `edges.yaml: spread_scaling`) and sits
inside the accepted bands. Verified facts to preserve: `supply_line_weight = 0` is unstable at every accelerator
...
- **Spreads (price channel):** `spread_j = base_j + (nd_j/2.5)·[s_gate·(1 − g) + s_loss·(ll − ll0)]`, `s_gate` 0.04,
  `s_loss` 2.0; household spread moves with the mean. Spreads enter `cc_gap_j`, the residential rate gap and R2.
...
| monetary | `z_mon` | residual on the policy rate | output −, inflation − |
...
policy: {elb: 0.0, core_weight: 0.0}
...
credit: {gate_enabled: false, s_gate: 0.04, s_loss: 2.0, collateral_trend_years: 5}
...
(nd-independent), +100bp spread at nd = 5 → −0.90 pp; supply line: doubling the pipeline lowers the rate; completions
...
trade balance posted; `ROW` accumulates `DEP`/`GBOND` when trade is unbalanced (no FX in v1). **Tests:** balanced at
